...
WHERE        (INVENTLOCATIONTYPE = 0) AND (SHIPCPYCONTAINERCLOSED = 0)GO
...
INNER JOIN  DAXINVENTSUMINVENTDIMVIEW T2 ON T1.INVENTLOCATIONID = T2.INVENTLOCATIONIDGO
...
LEFT OUTER JOIN
  (SELECT T1.FROMUNITOFMEASURE,
   ...
   AND T3.SYMBOL = 'LBS') AS UOM ON IT.PRODUCT = UOM.PRODUCTGO
...
T2.NAME
FROM INVENTTABLE T1
INNER JOIN ECORESPRODUCTTRANSLATION T2 ON T1.PRODUCT = T2.PRODUCTGO
...
WHERE T2.SHIPCPYCONTAINERCLOSED = 0 GO
