...
GO
IF EXISTS (SELECT TABLE_NAME FROM INFORMATION_SCHEMA.VIEWS  WHERE TABLE_NAME = 'DAXENTORNO') DROP VIEW DAXENTORNO
GO
CREATE VIEW [dbo].[DAXENTORNO] AS
SELECT 
	TOP 1
	SUBSTRING(SERVERID,4,10) SERVIDOR,
	CASE
	WHEN SUBSTRING(SERVERID,4,10) = 'RD2' THEN 'DESARROLLO'
	WHEN SUBSTRING(SERVERID,4,10) = 'DES' THEN 'PRUEBAS'
	WHEN SUBSTRING(SERVERID,4,10) = 'WEB' THEN 'PRODUCCION'
	WHEN SUBSTRING(SERVERID,4,10) = 'WEB02' THEN 'PRODUCCION'
	END ENTORNO
...
AND (T5.STOPPED = 0)
GO
IF EXISTS (SELECT TABLE_NAME FROM INFORMATION_SCHEMA.VIEWS  WHERE TABLE_NAME = 'DAXINVENTBATCHVALID') DROP VIEW DAXINVENTBATCHVALID
GO
...
AND SCCT.SHIPCPYCONTAINERSTATE NOT IN (5,8)
...
GO
IF EXISTS (SELECT TABLE_NAME FROM INFORMATION_SCHEMA.VIEWS  WHERE TABLE_NAME = 'DAXINVENTONHANDITEM') DROP VIEW DAXINVENTONHANDITEM
GO
CREATE VIEW [dbo].[DAXINVENTONHANDITEM] AS
SELECT IT.ITEMID,
       IT.ITEMGROUPID,
       IT.NAME,
       IT.PDSCWUNITID,
...
SELECT PARKINGSPACEID,
  ...
  (SELECT COUNT(*) AS Expr1
   ...
   WHERE (T1.PARKINGSPACEID = dppt.PARKINGSPACEID)
     AND dppt.PARKINGINACTIVE = 0
     AND (dppt.PARKINGSPACETRANSTYPE in (4,3,9,7))) AS TAKENPOSITION,
  ...
  (SELECT COUNT(*) AS Expr1
   FROM dbo.DAXPARKINGPOSITIONTABLE AS dppt
   WHERE (T1.PARKINGSPACEID = dppt.PARKINGSPACEID)) AS TOTALPOSITION,
  ...
  (SELECT COUNT(*) AS Expr1
   ...
   WHERE (T1.PARKINGSPACEID = dppt.PARKINGSPACEID)
     AND dppt.PARKINGINACTIVE = 0
     AND (dppt.SHIPPINGCPYTYPE = 0)
	 AND (dppt.PARKINGSPACETRANSTYPE in (4,3,9,7))) AS COMMERCIALCONTAINERS,
  ...
  (SELECT COUNT(*) AS Expr1
   ...
   WHERE (T1.PARKINGSPACEID = dppt.PARKINGSPACEID)
     AND dppt.PARKINGINACTIVE = 0
     AND (dppt.SHIPPINGCPYTYPE = 1)
	 AND (dppt.PARKINGSPACETRANSTYPE in (4,3,9,7))) AS OWNCONTAINERS,
  ...
  (SELECT COUNT(*) AS Expr1
   FROM
     (SELECT dppt.SHIPCPYCONTAINERID
	  ...
	  AND dppt.PARKINGINACTIVE = 0
	  AND (dppt.PARKINGSPACETRANSTYPE in (4,3,9,7))
...
SELECT ST.SALESNAME AS SALESNAME_OLD,
       ...
       SL.SALESUNIT,
       IT.QTY,
       ABS(IT.PDSCWQTY) AS PDSCWQTYRES,
	   ABS(IT.PDSCWQTY) AS QTYRES,
       AL.INVENTLOCATIONID,
       AL.INVENTBATCHID,
       AL.WMSLOCATIONID,
...
SELECT T1.SHIPCPYCONTAINERLOCALPORTID, T1.NAME, T1.DATAAREAID 
...
GO
IF EXISTS (SELECT TABLE_NAME FROM INFORMATION_SCHEMA.VIEWS  WHERE TABLE_NAME = 'VW_DAXINVENT_TRANSFERJOURNAL_AVAIL') DROP VIEW VW_DAXINVENT_TRANSFERJOURNAL_AVAIL
GO
CREATE VIEW VW_DAXINVENT_TRANSFERJOURNAL_AVAIL 
AS 
select distinct DATAAREAID, INVENTLOCATIONID
from DAXINVENT_TRANSFERJOURNAL
where PDSCWPHYSICALINVENT = 0
    AND AVAILPHYSICAL > 0 OR AVAILPHYSICAL > 0
    AND PDSCWPHYSICALINVENT > 0
GO