...
SELECT        A.PARKINGSPACEID, B.DATAAREAID, B.PARKINGSECTORID, C.PARKINGPOSITIONID, C.NAME, D.SHIPCPYCONTAINERID, D.SHIPCPYCUSTOMMARKID, D.SHIPCPYCONTAINERSTATE, D.SHIPCPYEXPECTEDDATE, 
                         E.INTRASTATTRANSPORT
FROM            dbo.PARKINGSPACETABLE AS A INNER JOIN
                         dbo.PARKINGSECTORTABLE AS B ON A.DATAAREAID = B.DATAAREAID INNER JOIN
                         dbo.PARKINGPOSITIONTABLE AS C ON B.RECID = C.PARKINGSECTORTABLERECID INNER JOIN
                         dbo.SHIPCPYCONTAINERTABLE AS D ON D.DATAAREAID = A.DATAAREAID INNER JOIN
                         dbo.SHIPPINGCPYVENDTABLE AS E ON E.RECID = D.SHIPPINGCPYVENDTABLERECID
...
SELECT        A.SHIPCPYCONTAINERID, A.SHIPCPYCONTAINERSTATE, A.SHIPCPYCUSTOMMARKID, A.SHIPCPYEXPECTEDDATE, A.SHIPCPYREALDATETIME, A.DATAAREAID, B.NAME, B.PARKINGPOSITIONID, 
                         B.PARKINGSPACETRANSTYPE, C.PARKINGSECTORID, D.PARKINGSPACEID, D.NAME AS SITIO, E.INTRASTATTRANSPORT
FROM            dbo.SHIPCPYCONTAINERTABLE AS A INNER JOIN
                         dbo.PARKINGPOSITIONTABLE AS B ON A.RECID = B.SHIPCPYCONTAINERTABLERECID INNER JOIN
                         dbo.PARKINGSECTORTABLE AS C ON B.PARKINGSECTORTABLERECID = C.RECID INNER JOIN
                         dbo.PARKINGSPACETABLE AS D ON D.RECID = C.PARKINGSPACETABLERECID INNER JOIN
                         dbo.SHIPPINGCPYVENDTABLE AS E ON E.RECID = A.SHIPPINGCPYVENDTABLERECID
...
CREATE VIEW [dbo].[DAXEMPTYWAREHOUSE]
AS SELECT DISTINCT 
T1. USERID,
T1.INVENTLOCATIONID,
T1.DATAAREAID
...
FROM            dbo.INVENTTABLE AS T1 INNER JOIN
                         ...
                         dbo.ECORESPRODUCTTRANSLATION AS T4 ON T1.PRODUCT = T4.PRODUCT AND T1.PARTITION = T4.PARTITION INNER JOIN 
						 dbo.INVENTITEMINVENTSETUP AS T5 ON T1.ITEMID = T5.ITEMID AND T1.DATAAREAID = T5.DATAAREAID AND T1.PARTITION = T5.PARTITION
WHERE        (T4.LANGUAGEID = N'es-mx')
				AND (T5.INVENTDIMID = 'AllBlank')
				AND (T5.STOPPED = 0)
...
SELECT        T1.PARKINGSPACEID, T1.PARKINGSPACENAME, T1.PARKINGSECTORID, T1.PARKINGSECTORNAME, T1.PARKINGPOSITIONID, T1.SHIPCPYCONTAINERID, T1.SHIPCPYEXPECTEDEXITDATE, T1.PARKINGSPACETRANSTYPE, 
                         T1.PARKINGSPACETRANSTYPE_DESC, T1.MAXWEIGHT, ISNULL(T2.INVENTQTYAVAILPHYSICAL, 0) AS INVENTQTYAVAILPHYSICAL,
                         CASE WHEN T2.MAXWEIGHT != 0 THEN ROUND((T2.INVENTQTYAVAILPHYSICAL / T2.MAXWEIGHT) * 100, 2) ELSE 0 END AS PERCENTFILLCONTAINER
FROM            dbo.DAXPARKINGPOSITIONTABLE AS T1 LEFT OUTER JOIN
                             (SELECT        SHIPCPYCONTAINERID, SHIPCPYCONTAINERRECID, SHIPPINGCPYTYPE, SUM(INVENTQTYPHYSICALONHAND) AS INVENTQTYPHYSICALONHAND, SUM(INVENTQTYAVAILPHYSICAL) 
                                                         AS INVENTQTYAVAILPHYSICAL, SUM(INVENTQTYRESERVPHYSICAL) AS INVENTQTYRESERVPHYSICAL, SUM(PdsCWAvailPhysical) AS PDSCWAVAILPHYSICAL, SUM(PdsCWPhysicalInvent) 
                                                         AS PDSCWPHYSICALINVENT, SUM(POSTEDVALUE) AS POSTEDVALUE, SUM(COSTPRICE) AS COSTPRICE, SUM(PDSCWCOSTPRICE) AS PDSCWCOSTPRICE, MIN(MAXWEIGHT) 
                                                         AS MAXWEIGHT
                               FROM            dbo.DAXINVENTONHANDCONTAINER
                               GROUP BY SHIPCPYCONTAINERID, SHIPCPYCONTAINERRECID, SHIPPINGCPYTYPE) AS T2 ON T1.SHIPCPYCONTAINERRECID = T2.SHIPCPYCONTAINERRECID
...
SELECT 
	iif(t2.QtyConvert01ByUnitAvg > 0,(t2.INVENTQTYAVAILPHYSICAL/t2.QtyConvert01ByUnitAvg)*100,0) as PERCENTFILLPALLET, 
...
) as t2 on t1.shipcpycontainerrecid = t2.shipcpycontainerrecid and t1.wmslocationid = t2.wmslocationid
...
IF EXISTS (SELECT TABLE_NAME FROM INFORMATION_SCHEMA.VIEWS  WHERE TABLE_NAME = 'DAXPARKINGPOSITIONTABLE2') DROP VIEW DAXPARKINGPOSITIONTABLE2
 GO 
CREATE VIEW DAXPARKINGPOSITIONTABLE2 AS
select DISTINCT
...
from  
       ...
       PARKINGPOSITIONTABLE ppt 

       inner join ParkingSectorTable pset on ppt.PARKINGSECTORTABLERECID = pset.RECID and pset.PARKINGINACTIVE = 0 
...
PL.PARTITION,
PL.RECID
from dbo.PURCHLINE AS PL INNER JOIN
dbo.INVENTTABLE AS ITL ON PL.ITEMID = ITL.ITEMID AND PL.DATAAREAID = ITL.DATAAREAID INNER JOIN
dbo.ECORESPRODUCTTRANSLATION AS ERPT ON ITL.PRODUCT = ERPT.PRODUCT AND ITL.PARTITION = ERPT.PARTITION
WHERE 
(ERPT.LANGUAGEID = N'es-mx');


IF EXISTS (SELECT TABLE_NAME FROM INFORMATION_SCHEMA.VIEWS  WHERE TABLE_NAME = 'DAXSALESORDERVIEW') DROP VIEW DAXSALESORDERVIEW
 GO 
CREATE VIEW [dbo].[DAXSALESORDERVIEW]
AS
SELECT        ST.SALESNAME AS SALESNAME_OLD, DPT.NAME AS SALESNAME, SL.SALESID, SL.LINENUM, SL.ITEMID, SL.DATAAREAID, SL.SALESQTY, SL.SALESPRICE, SL.QTYORDERED, 
                         SL.PDSCWQTY AS PDSCWQTYSALE, SL.SALESUNIT, IT.QTY, IT.PDSCWQTY AS PDSCWQTYRES, AL.INVENTLOCATIONID, AL.INVENTBATCHID, AL.WMSLOCATIONID, SL.NAME AS NAME_DEFAULT, SL.LINEAMOUNT,
						 ERPT.NAME
FROM            dbo.SALESLINE AS SL INNER JOIN
                         dbo.INVENTTRANSORIGIN AS ITO ON ITO.PARTITION = SL.PARTITION AND ITO.DATAAREAID = SL.DATAAREAID AND ITO.INVENTTRANSID = SL.INVENTTRANSID INNER JOIN
                         dbo.INVENTTRANS AS IT ON IT.PARTITION = ITO.PARTITION AND IT.DATAAREAID = ITO.DATAAREAID AND IT.INVENTTRANSORIGIN = ITO.RECID INNER JOIN
                         dbo.INVENTDIM AS AL ON IT.INVENTDIMID = AL.INVENTDIMID AND IT.PARTITION = AL.PARTITION INNER JOIN
						 dbo.INVENTTABLE AS ITL ON SL.ITEMID = ITL.ITEMID AND SL.DATAAREAID = ITL.DATAAREAID INNER JOIN
						 dbo.ECORESPRODUCTTRANSLATION AS ERPT ON ITL.PRODUCT = ERPT.PRODUCT AND ITL.PARTITION = ERPT.PARTITION INNER JOIN
                         dbo.SALESTABLE AS ST ON SL.SALESID = ST.SALESID INNER JOIN
                         dbo.CUSTTABLE AS CT ON ST.CUSTACCOUNT = CT.ACCOUNTNUM AND ST.DATAAREAID = CT.DATAAREAID INNER JOIN
                         dbo.DIRPARTYTABLE AS DPT ON CT.PARTY = DPT.RECID
WHERE        (SL.PARTITION = 5637144576) AND (IT.STATUSISSUE = 4)
AND ERPT.LANGUAGEID = N'es-mx'
--and  SL.SALESID = 'OV-024477'
...
CREATE VIEW [dbo].[DAXWAREHOUSEUSER]
AS SELECT 
T1. USERID,
T1.INVENTLOCATIONID,
T1.DATAAREAID,
...
WHERE
		T1.SHIPCPYCONTAINERRECID IS NOT NULL
...
IF EXISTS (SELECT TABLE_NAME FROM INFORMATION_SCHEMA.VIEWS  WHERE TABLE_NAME = 'VIEW1') DROP VIEW VIEW1
 GO 
CREATE VIEW "DBO".VIEW1 AS SELECT T1.RECID AS RECID1,T1.ITEMID AS ITEMID,T1.DATAAREAID AS DATAAREAID,T1.PARTITION AS PARTITION,T1.RECID AS RECID,T2.PARTITION AS PARTITION#2,T3.PARTITION AS PARTITION#3,T3.LANGUAGEID AS LANGUAGEID,T3.NAME AS NAME FROM INVENTTABLE T1 CROSS JOIN ECORESPRODUCT T2 CROSS JOIN ECORESPRODUCTTRANSLATION T3 WHERE (T1.PRODUCT=T2.RECID AND (T1.PARTITION = T2.PARTITION)) AND (T3.PRODUCT=T2.RECID AND (T3.PARTITION = T2.PARTITION))
