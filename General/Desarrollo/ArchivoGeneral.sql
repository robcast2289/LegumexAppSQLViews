  ...
  (SELECT COUNT(*) AS Expr1
   FROM
     (SELECT dppt.SHIPCPYCONTAINERID
	  ...
	  AND dppt.PARKINGINACTIVE = 0
	  AND (dppt.PARKINGSPACETRANSTYPE in (4,3,9,10,7))
...
FROM SHIPCPYCONTAINERLOCALPORT T1GO
...
FROM SHIPCPYCONTAINERUNIT T1GO
...
AND T1.PdsCWPhysicalInvent > 0
AND TBL2.PARKINGSPACETRANSTYPE IN (3,4,7,9,10)
AND TBL3.SHIPCPYCONTAINERCLOSED = 0) AS T2
